...
def sqlcheck(url):
    ...
    for (dbms, regx) in ((dbms, regx) for dbms in DBMS_ERRORS for regx in Downloader[dbms]):
        if (re.search(regx, _content)):
            return True
    content = {}
    content["origin"] = Downloader.get(_url)
    for test_payload in BOOLEAN_TESTS:
        RANINT = random.randint(1, 255)
        _url = url + test_payload%(RANINT, RANINT)
        content["true"] = Downloader.get(_url)
        _url = url + test_payload%(RANINT, RANINT+1)
